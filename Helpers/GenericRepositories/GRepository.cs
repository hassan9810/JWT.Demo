...
namespace JWT.Demo.Helpers.GenericRepositories
{
         ...
         where T : class
    {
        ...
        public virtual IQueryable<T> GetAll(Expression<Func<T, bool>> where, int pageIndex, int pageSize, ref ResponseDTO responseDto)
        {
            var data = _dbContext.Set<T>().AsNoTracking().Where(where).AsQueryable();
            var items = data.Skip((pageIndex - 1) * pageSize).Take(pageSize).AsQueryable();
            var count = data.Count();

            responseDto.PageIndex = pageIndex;
